...
for product in config:
    ...
    for index, version in enumerate(config[product]["versions"]):
        ...
        # Delete tags
        check_tag = hammock("https://api.github.com/repos/dhinakg/ktextrepo-beta/git/refs/tags/" + product + "-" + version["commit"]["sha"][:7], auth=("dhinakg", token))
        ...
        for i in version["links"]:
            local_path = Path(version["links"][i].replace("https://raw.githubusercontent.com/dhinakg/ktextrepo/builds", "Builds"))
