...
for plugin in plugins:
    ...
    for commit in commits:
        ...
        if (commit_date >= date_to_compare or (plugin.get("Force") and commits.index(commit)  == 0)) and not matched_key_in_dict_array(config.get(repo, {}).get("versions", []), "commit", commit["sha"]) and commits.index(commit) <= 5:
